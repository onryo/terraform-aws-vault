...
resource "aws_security_group_rule" "allow_api_inbound_from_cidr_blocks" {
  count       = length(var.allowed_api_inbound_cidr_blocks) >= 1 ? 1 : 0
  type        = "ingress"
  from_port   = var.api_port
  to_port     = var.api_port
  protocol    = "tcp"
  cidr_blocks = var.allowed_api_inbound_cidr_blocks
...
resource "aws_security_group_rule" "allow_cluster_inbound_cidr_blocks" {
  count       = length(var.allowed_cluster_inbound_cidr_blocks) >= 1 ? 1 : 0
  type        = "ingress"
  from_port   = var.cluster_port
  to_port     = var.cluster_port
  protocol    = "tcp"
  cidr_blocks = var.allowed_cluster_inbound_cidr_blocks
